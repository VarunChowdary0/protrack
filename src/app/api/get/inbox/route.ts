...
export async function GET(request: Request) {
    try{
        ...
        .leftJoin(inboxAttachments, eq(inbox.id, inboxAttachments.inboxId)) // attachments
        .where(eq(inbox.userId, reqFromUser.user.id));        
        
        const formattedInbox = user_inbox.map((inboxItem) => {
        ...
        return {
            ...
            task: inboxItem.tasks ? {
                ...inboxItem.tasks
            } : undefined,
            calendarId: inboxData.calendarId,
            calendarEvent: inboxItem.calendar ? {
                ...inboxItem.calendar
            } : undefined,
            ...
            attachments: inboxItem.inbox_attachments ? [{
                id: inboxItem.inbox_attachments.id,
                name: inboxItem.inbox_attachments.name,
                type: inboxItem.inbox_attachments.type,
                size: inboxItem.inbox_attachments.size
            }] : []
        };
    });
